...
async def score_example(
    ...
    async with semaphore:
        results = await asyncio.gather(
            *[
                score_response(
                    client, prompt, resp, subset, model=model, temperature=temperature
                )
...
async def main():
    parser = argparse.ArgumentParser(description="Run RB2 baselines")
    parser.add_argument(
        "--model",
    ...
    for subset in subsets_sorted:
        ...
        while valid_per_subset[subset] < subset_target and queue_idx < len(queue):
            # Grab a batch of remaining needed examples
    ...
    output_file = output_dir / f"baseline_{model_short}.jsonl"
