...
\icmltitlerunning{Practical LLM-as-a-Judge Techniques on RewardBench~2}

\begin{document}

\twocolumn[
\icmltitle{An Empirical Investigation of Practical LLM-as-a-Judge Improvement Techniques on RewardBench~2}
...
\begin{abstract}
LLM-as-a-judge --- using a language model to score or rank candidate responses --- is widely used as a scalable alternative to human evaluation in RLHF pipelines, benchmarking, and application layer evaluations (evals). However, judgment reliability depends heavily on prompting and aggregation strategy. We present an empirical investigation of practical, drop-in techniques that improve GPT-5.4 judge accuracy on RewardBench~2, with a unifying lens of \emph{noise control on the stochastic judge}: ensembling as Monte Carlo averaging over per-call noise, criteria injection as between-response discrimination sharpening, and per-response score variance as a (weak but measurable) uncertainty signal. Two techniques account for nearly all available gains: task-specific criteria injection (+3.0pp at negligible cost) and ensemble scoring (+9.8pp at 5$\times$ cost). Combined, they reach \textbf{83.6\%} accuracy, +11.9pp over the 71.7\% baseline. Our investigation also covers three further techniques (calibration context, adaptive model escalation, and soft blending) which did not reliably improve on criteria + ensembling at comparable cost. Cheaper model tiers benefit disproportionately from ensembling: GPT-5.4 mini with $k{=}8$ achieves 79.2\% at 1.2$\times$ baseline cost, making high-accuracy LLM judges accessible at low cost.
\end{abstract}
...
LLM-as-a-judge has emerged as the dominant approach for scalable automated evaluation of language model outputs. A judge model rates or ranks candidate responses, providing a signal that can be used for reward modelling and benchmarking. Judges are also deployed as offline test suites that gate releases on output quality, and as real-time monitors that flag regressions or policy violations in deployed applications. Despite this wide adoption, the reliability of LLM judges varies considerably across prompting strategies and aggregation methods.

At temperature $>0$, an LLM judge is a \emph{stochastic scorer}. We adopt this as a unifying lens: ensembling reduces per-call score variance via Monte Carlo averaging, and criteria injection sharpens between-response discrimination (the tie rate on RB2 drops from 20.4\% to 17.4\% at $k{=}1$ and from 4.5\% to 3.2\% at $k{=}8$). Techniques that attempt to add new information or reroute to a different estimator do not improve on this simple baseline once $k$ is large. Per-response score variance also provides a weak but measurable uncertainty signal (AUC${\approx}$0.60 for predicting incorrect judgments), which we examine as both a routing target and a diagnostic for judge error.
...
We additionally evaluate a \textbf{combined} condition that stacks all four to test additivity. Our empirical investigation finds that criteria injection and ensembling account for nearly all available gains, while calibration, model routing, and soft blending do not reliably improve on this simpler baseline at comparable cost. We report results for all conditions to provide a complete picture of what works in practice. All experiments are conducted with GPT-5.4 (full), GPT-5.4 mini, and GPT-5.4 nano via Azure OpenAI.

\paragraph{Contributions.} Our main contributions are:
\begin{itemize}
  \item A systematic comparison of four drop-in techniques for improving LLM judge accuracy (plus a combined stacking condition), evaluated on 1,753 RewardBench~2 examples across three model tiers (GPT-5.4, GPT-5.4 mini, GPT-5.4 nano).
  \item Evidence that criteria injection and ensembling dominate the cost--accuracy tradeoff, reaching 83.6\% (+11.9pp) at 5.3$\times$ cost, while calibration, model routing, and soft blending improved over baseline but were dominated by criteria + ensembling at comparable or lower cost.
  \item A cost--accuracy analysis showing that cheaper models benefit disproportionately from ensembling: mini $k{=}8$ matches the full model's single-shot accuracy at 1.2$\times$ cost.
\end{itemize}
...
\begin{table}[ht]
\centering
\caption{Model pricing (OpenAI API, April 2026).}
\label{tab:models}
\small
\begin{tabular}{lrrl}
\toprule
...
\paragraph{Temperature and reasoning effort.} We use temperature 1.0 (a sweep in Section~\ref{sec:temperature} confirms ensembling helps at all temperatures, with gains growing from +4.6pp at temp=0 to +9.8pp at temp=1.0) and set \texttt{reasoning\_effort="none"} for all conditions to isolate prompt-level and aggregation effects.
...
\paragraph{Method.} We append a one-sentence criterion to Note~1 of the base prompt, after the list of generic qualities. Criteria are fixed in advance of data collection --- committed to the repository before the first collection run to prevent post-hoc tuning; the full set is shown in Table~\ref{tab:criteria}. (This is an internal-repository commitment, not an external pre-registration registry.)
...
Note~1 of the prompt is modified as follows (shown for Math; other categories substitute their criterion from Table~\ref{tab:criteria}, addition in \textbf{bold}):
...
GPT-5.4 mini is ${\sim}3\times$ cheaper than the full model but somewhat less accurate. If we could identify which examples the mini model will get wrong, we could route only those to the full model. We investigate one such \emph{technique} --- variance-based escalation --- evaluated in three variants (hard routing, soft blending, variance-informed adaptive ensembling); we ultimately do not recommend any of the three, but the analysis shows \emph{why} and clarifies what role per-response variance plays as an uncertainty signal. We use the mini model's per-response score variance as the routing signal throughout.

\paragraph{Variance as an error signal.} Per-response variance correlates weakly but systematically with correctness ($r = -0.13$, Figure~\ref{fig:variance-error}), and mini-model variance tracks full-model variance with $r = 0.421$, so mini variance is a \emph{detectable} (if noisy) uncertainty proxy; nano variance is much more weakly correlated ($r = 0.106$, Appendix~\ref{sec:escalation-detail}). Whether that signal can be \emph{exploited} by a routing scheme is a separate question: in RewardBench~2's best-of-4 setting, winner selection depends on the \emph{ordinal} ranking of four response means, so per-response variance must cross a large threshold to flip the winner --- a condition that holds only at the extremes, producing the Pareto ``dead zone'' we observe in Section~\ref{sec:main-results}.

\begin{figure}[t]
\centering
\includegraphics[width=\columnwidth]{variance_error_signal.png}
\caption{Distribution of ensemble score variance for correct vs incorrect judgments (baseline $k{=}8$, full model). Incorrect judgments exhibit modestly higher variance ($r{=}{-}0.13$). As a classifier of incorrectness, per-response variance reaches AUC${=}0.60$ with precision@recall(20\%)${=}0.32$ (base rate 0.19): a weak but statistically meaningful uncertainty signal.}
\label{fig:variance-error}
\end{figure}

\paragraph{Strategies.} We investigate three escalation strategies that use this signal: \emph{hard variance routing} (binary per-response routing above a variance threshold), \emph{sigmoid-weighted soft blending} (continuous per-response blend of mini and full scores), and \emph{variance-informed adaptive ensembling} (variable number of full-model calls per response). All are evaluated offline on paired mini/full scores ($k{=}8$ each) collected in a single pass, requiring no additional API calls. Full method descriptions, equations, and correlation plots are in Appendix~\ref{sec:escalation-detail}.
...
\begin{table*}[!htbp]
\centering
\caption{Main results by condition and category. \textbf{(a)} Techniques that reliably improve accuracy. \textbf{(b)} Investigated techniques that did not improve on criteria $k{=}8$ at comparable cost. Full calibration variant breakdowns are in Appendix~\ref{sec:calibration-detail}.}
...
\begin{figure}[t]
\centering
\includegraphics[width=\columnwidth]{hero_accuracy.png}
\caption{Accuracy by condition and category. Criteria $k{=}8$ (83.6\%) matches or slightly exceeds combined (82.6\%) at lower cost (5.3$\times$ vs 6.8$\times$ baseline); their CIs overlap and we interpret the ordering as a practical tie. Precise~IF remains the hardest category across all conditions.}
...
\caption{Cost vs accuracy Pareto frontier. Criteria $k{=}8$ Pareto-dominates all other high-accuracy conditions; nano $k{=}8$ dominates the low-cost frontier.}
\label{fig:pareto}
\end{figure}

\paragraph{Headline results.} The baseline reaches 71.7\% ($\pm$2.1pp) at \$0.013/ex. Criteria injection alone adds +3.0pp at $k{=}1$ (74.7\%; paired bootstrap $\Delta{=}{+}2.9$pp, 95\% CI $[{+}1.1, {+}4.6]$, $p{>}0.999$) with negligible marginal cost --- a few extra input tokens per call, no structural change to the scoring protocol --- and modestly reduces the tie rate at $k{=}1$ (20.4\% baseline $\to$ 17.4\% criteria). Ensembling at $k{=}8$ adds +9.8pp (81.5\% at 5$\times$ cost) and sharply reduces the tie rate (baseline $k{=}1$: 20.4\%, $k{=}8$: 4.5\%), since ties now require all four response means to match exactly across $k$ draws. Together, criteria + ensembling reach \textbf{83.6\%} (+11.9pp over baseline, 5.3$\times$ cost) and further collapse the tie rate to 3.2\%, paired-significant against vanilla ensembling ($\Delta{=}{+}2.0$pp, 95\% CI $[{+}0.8, {+}3.3]$, $p{>}0.999$).

\paragraph{Task-specific criteria: where gains concentrate.} The $k{=}1$ criteria gain is carried largely by Math (+12.0pp) and Safety (+3.3pp); Focus and Factuality move within their per-category CIs ($\pm$4pp) and should be read as directional. Precise~IF actually regresses slightly at $k{=}1$ (32.1\% vs baseline 34.0\%, well within noise), but $k{=}8$ ensembling recovers it to 48.8\%, so the per-category picture is most stable when criteria is combined with ensembling (full per-category CIs in Appendix~\ref{sec:per-cat-ci}). Criteria were fixed in advance of data collection (committed to the internal repository before the first collection run) to reduce risk of post-hoc criterion selection; this is an internal-repo timestamp, not an external registry.

\paragraph{Cheaper models benefit disproportionately.} The practical operating-point story is mini + criteria: mini $k{=}8$ achieves 79.2\% at 1.2$\times$ baseline cost, and adding criteria pushes it to 81.5\%, \textbf{matching the full model's vanilla $k{=}8$ ensemble at roughly one-quarter the cost}. Nano $k{=}8$ sits at the extreme end of the cost-accuracy frontier: 71.4\% at 0.4$\times$ baseline cost approaches the full model's single-shot baseline, but at $k{=}8$ nano still trails mini $k{=}8$ by 7.8pp --- ensembling raises a lower-capability model's floor but not its ceiling. Ensembling's absolute per-call gain is largest for mini (+9.6pp from $k{=}1\to k{=}8$) consistent with (but not proof of) Jensen-type variance-reduction arguments; the nano $+$19.1pp gain benefits from both Jensen-type averaging and lower starting accuracy. Most of the gain is captured by $k{=}3$ for all tiers (Figure~\ref{fig:diminishing-returns}); beyond that, returns diminish.

\paragraph{Calibration context.} All four calibration variants improve over baseline at $k{=}1$ by +1--2pp. The ``low'' variant (anchoring to a rejected response) slightly outperforms ``high'' (anchoring to the chosen response, 73.8\% vs 72.4\%) --- possibly because the judge finds it easier to distinguish the target from a known-bad anchor than to match a known-good one. Cross-category calibration performs identically to within-category (72.4\%), suggesting the benefit is context length and anchoring rather than category-specific transfer. \textbf{At $k{=}8$, however, calibration provides no additional benefit beyond ensembling alone}: all variants fall within $\pm$0.2pp of vanilla ensembling (81.5\%, cal.\ low: 81.7\%). The $k{=}8$ ensemble already reduces scoring noise enough that the anchoring benefit is redundant --- contrast with criteria, which provides +2.1pp even at $k{=}8$.

\paragraph{Combined and blending.} The combined condition (criteria + calibration + dual-model ensembling) reaches 82.6\% at 6.8$\times$ baseline cost --- lower than criteria $k{=}8$ alone (83.6\% at 5.3$\times$), so stacking all interventions does not help. Soft blending achieves 83.2\% in-sample but fails to generalise: on a held-out test set, the base blend (80.2\%) does not beat full $k{=}8$ (81.5\%), indicating midpoint overfitting (Figure~\ref{fig:soft-blending-fig}). Variance-informed routing at low budget (74.9\% at 1.6$\times$ cost) is dominated by mini $k{=}8$ (79.2\% at 1.2$\times$). Hard variance routing traces a Pareto frontier with a large dead zone (Figure~\ref{fig:escalation-pareto}): escalating some but not all responses rarely changes the four-way winner, since accuracy depends on relative rankings, so meaningful operating points cluster near mini $k{=}8$ (cheap) or full $k{=}8$ (expensive) with little gain in between. Full calibration variant results are in Appendix~\ref{sec:calibration-detail}; full escalation analysis is in Appendix~\ref{sec:escalation-detail}.
...
\caption{Pareto frontier for per-response variance-informed ensembling. Gray points: full grid of $(\sigma_1, \sigma_2)$ configurations (train-set). Stars: test-set accuracy for best overall (81.0\%) and budget-constrained (74.9\%) configurations --- the latter is dominated by mini $k{=}8$ at comparable cost.}
...
\paragraph{Robustness.} Sample sizes differ slightly across conditions ($N{=}1700$--$1746$) due to Azure content-filter refusals. Recomputing headline conditions on the intersection of ${\sim}$1710 examples succeeding under all four main collections changes accuracies by at most 0.2pp and preserves the ranking (Appendix~\ref{sec:intersection}). Precise~IF is the hardest category across all conditions (26--53\%); the combined condition provides the largest absolute improvement here (+18.5pp over baseline).

\paragraph{Temperature sensitivity.} At $k{=}1$, accuracy is stable across temperatures (71--73\%, CIs overlapping). At $k{=}8$, ensembling helps at all temperatures, with gains growing from +4.6pp at temp=0 (72.5\% $\to$ 77.1\%) to +9.8pp at temp=1.0 (71.7\% $\to$ 81.5\%). Surprisingly, even at temp=0 there is a significant +4.6pp $k{=}1$ vs $k{=}8$ gap with non-overlapping CIs: \texttt{temperature=0} does \emph{not} produce deterministic outputs in practice, likely due to GPU floating-point non-determinism and the absence of a \texttt{seed} parameter. Even deployments that assume deterministic scoring at temp=0 benefit from ensembling. Full sweep plot in Appendix~\ref{sec:additional-analyses}.
...
\begin{table}[ht]
\centering
\caption{Prompt variant accuracy (\%) at $k{=}8$.}
...
Criteria + cal.\ (combined)       & 82.6 & 79.2 & --- \\
\bottomrule
\end{tabular}
\end{table}

Criteria is the only prompt technique that helps at $k{=}8$: +2.1pp for the full model, +2.3pp for mini. Calibration is a no-op at $k{=}8$ (+0.2pp, within noise), and \textbf{adding calibration to criteria actually hurts} --- the combined condition (82.6\%) is 1.0pp below criteria alone (83.6\%). Mini + criteria $k{=}8$ (81.5\%) matches full-model vanilla $k{=}8$ at one-quarter the cost, the optimal operating point for cost-constrained deployments.
...
\caption{Model agreement with full ($k{=}8$) as a function of ensemble size. Mini plateaus at 78.7\% by $k{=}5$; nano plateaus at ${\sim}$70\%, with a lower rank-correlation ceiling (0.67 vs 0.79 for mini), confirming a larger capability gap.}
\label{fig:convergence}
\end{figure}

\paragraph{Criteria + ensembling as a simple strong baseline.} Criteria $k{=}8$ (83.6\%) matches or slightly exceeds the full combined condition (82.6\%) at lower cost (5.3$\times$ vs 6.8$\times$); if you are already paying for $k{=}8$, adding criteria is a near-free +2.1pp upgrade over vanilla ensembling.
...
\begin{itemize}
  \item All experiments use a single judge model family (GPT-5.4); generalisability to other models (Claude, Gemini, open-weight) is untested.
  ...
  \item We set \texttt{reasoning\_effort="none"} throughout; we have not sanity-checked whether the +11.9pp criteria+ensembling gap persists at higher reasoning effort, and note that reasoning itself is a variance-reducing intervention that could substitute for ensembling at higher per-call cost.
...
\paragraph{Interpretation: noise control, not new judgment.} Ensembling is Monte Carlo variance reduction: per-call variance drops as $1/k$ and the $k{=}1 \to k{=}8$ tie-rate collapse (20.4\% $\to$ 4.5\%) is its observable signature. Criteria injection does \emph{not} change per-response score variance (mean $\sigma_i{=}0.31$ vs 0.32; KS $p{=}0.88$) but sharpens \emph{between-response} discrimination (tie rate 4.5\% $\to$ 3.2\% at $k{=}8$; AUC for variance predicting incorrectness rises from 0.60 to 0.64). Techniques that don't work (calibration, model routing, soft blending) inject new information or combine heterogeneous distributions; at high $k$, residual variance is small enough that these contribute noise rather than signal.

\paragraph{Reproducibility.} All experiments were conducted via Azure OpenAI API version \texttt{2025-04-01-preview} using GPT-5.4, GPT-5.4 mini, and GPT-5.4 nano deployments. Data was collected in March--April 2026. All API calls use temperature 1.0 with no seed parameter, so individual scores are not deterministically reproducible. Our bootstrap CIs reflect sampling variance over examples, not run-to-run API variance: Azure can silently rotate deployments, and we have not re-collected a headline condition to quantify cross-run drift. We recommend downstream users of these techniques re-benchmark on their own deployment. Total API spend across all collections was approximately \$1,500. All collection scripts write results incrementally and support resume. Code and raw data are available at \url{https://github.com/composo-ai/llm-judge-criteria-ensembling}.

\paragraph{Conclusion.} On RewardBench~2, two drop-in techniques dominate the cost--accuracy tradeoff for GPT-family judges: \textbf{criteria injection} (+3.0pp at $k{=}1$, near-free) and \textbf{ensembling} (+9.8pp at $k{=}8$, with $k{=}3$ capturing ${\sim}$70\% of the gain); combined they reach \textbf{83.6\%} (+11.9pp). Three other techniques (calibration, soft blending, variance-informed routing) are dominated at comparable cost; do not stack speculatively, as at high $k$ variance-reduction techniques become substitutes.
...
\paragraph{Motivation.} GPT-5.4 mini is ${\sim}3\times$ cheaper than the full model but somewhat less accurate. If we could identify in advance which examples the mini model will get wrong, we could route only those to the full model. We use the mini model's score variance as a proxy for example difficulty.
...
\paragraph{Variance as a routing signal.} Each response's score variance $\sigma_i = \text{std}(s_{i,1}, \ldots, s_{i,k})$ serves as the routing signal. Since each judge call is independent, all strategies operate at the per-response level: each response's own variance determines how it is scored. The mini-vs-full correlation story is summarised in the main paper (Figure~\ref{fig:variance-error}); for completeness, Figure~\ref{fig:variance-corr} below shows the cross-tier variance correlation. Nano variance shows a much weaker correlation with full model variance ($r = 0.106$) than mini ($r = 0.421$), suggesting that nano's scoring noise is largely independent and its uncertainty is not a useful proxy for the full model's.
...
\begin{tabular}{lccc}
\toprule
Strategy & Acc.\ & \$/ex & vs $k{=}1$ full \\
\midrule
$k{=}1$ full (baseline)            & 71.7\% & \$0.0133 & 1.0$\times$ \\
Full $k{=}8$                       & 81.5\% & \$0.0663 & 5.0$\times$ \\
Soft blend (full dataset, tuned)$^\dagger$ & 83.2\% & \$0.0817 & 6.1$\times$ \\
Soft blend (test set)              & 80.2\% & \$0.0817 & 6.1$\times$ \\
Var-informed ($\leq$2, test)       & 74.9\% & ${\sim}$\$0.022 & 1.6$\times$ \\
\bottomrule
\end{tabular}
...
Figure~\ref{fig:escalation-pareto} in the main paper shows the hard variance routing Pareto frontier, with its characteristic dead zone in the middle --- escalating some but not all responses rarely changes the four-way winner, so meaningful operating points cluster at the mini-only or full-only extremes.

On the full dataset (in-sample), soft blending achieves 83.2\% vs 81.5\% for full $k{=}8$ (see Figure~\ref{fig:soft-blending-fig} in the main paper). However, on a held-out test set the base blend (80.2\%) does not beat full $k{=}8$ (81.5\%), indicating midpoint overfitting; we therefore do not report blend numbers as a positive result in the main paper.

The budget-constrained variance-informed variant restricts mean $n_{\text{full}} \leq 2.0$, achieving 74.9\% on the test set at 1.6$\times$ baseline cost (see Figure~\ref{fig:var-informed} in the main paper) --- barely above the baseline (71.7\%) and far below mini $k{=}8$ (79.2\% at 1.2$\times$).
