...
\begin{abstract}
LLM-as-a-judge --- using a language model to score or rank candidate responses --- is widely used as a scalable alternative to human evaluation in RLHF pipelines, benchmarking, and application layer evaluations (evals). However, judgment reliability depends heavily on prompting and aggregation strategy. We present an empirical investigation of practical, prompt-only drop-in techniques that improve GPT-5.4 judge accuracy on RewardBench~2, with a unifying lens of \emph{noise control on the stochastic judge}: ensembling as Monte Carlo averaging over per-call noise, criteria injection as between-response discrimination sharpening, and per-response score variance as a (weak but measurable) uncertainty signal. Two techniques account for nearly all available gains: task-specific criteria injection (+3.0pp at negligible cost) and ensemble scoring (+9.8pp at 5$\times$ cost). Together, they reach \textbf{83.6\%} accuracy, +11.9pp over the 71.7\% baseline (+11.9pp also on the $N{=}1710$ intersection of all collections; see Appendix~\ref{sec:intersection}). Our investigation also covers three further techniques (calibration context, adaptive model escalation, and soft blending) which did not reliably improve on criteria + ensembling at comparable cost. Cheaper model tiers benefit disproportionately from ensembling: GPT-5.4 mini with $k{=}8$ achieves 79.2\% at 1.2$\times$ baseline cost, making high-accuracy LLM judges accessible at low cost.
\end{abstract}
...
\begin{itemize}
  ...
  \item Evidence that criteria injection and ensembling dominate the cost--accuracy tradeoff, reaching 83.6\% (+11.9pp, identical on the $N{=}1710$ intersection) at 5.3$\times$ cost, while calibration, model routing, and soft blending improved over baseline but were dominated by criteria + ensembling at comparable or lower cost.
  \item A cost--accuracy analysis showing that cheaper models benefit disproportionately from ensembling: mini $k{=}8$ reaches 79.2\% at 1.2$\times$ baseline cost, and mini+criteria $k{=}8$ matches full-model vanilla $k{=}8$ ensemble accuracy (81.5\%) at roughly one-quarter the cost.
\end{itemize}
...
\paragraph{Motivation.} GPT-5.4 mini is ${\sim}3\times$ cheaper than the full model but somewhat less accurate. If we could identify which examples the mini model will get wrong, we could route only those to the full model. Per-response score variance is a plausible routing signal: variance correlates weakly but systematically with correctness ($r{=}{-}0.13$, Figure~\ref{fig:variance-error}), and mini-model variance tracks full-model variance ($r{=}0.421$; nano's correlation is weaker at $r{=}0.106$, Appendix~\ref{sec:escalation-detail}).

\begin{figure}[t]
\centering
\includegraphics[width=\columnwidth]{variance_error_signal.png}
\caption{Distribution of ensemble score variance for correct vs incorrect judgments (baseline $k{=}8$, full model). Incorrect judgments exhibit modestly higher variance ($r{=}{-}0.13$). As a classifier of incorrectness, per-response variance reaches AUC${=}0.60$: a weak but statistically meaningful uncertainty signal.}
...
\begin{table*}[!htbp]
\centering
\caption{Main results by condition and category. \textbf{(a)} Techniques that reliably improve accuracy. \textbf{(b)} Investigated techniques that did not improve on criteria $k{=}8$ at comparable cost. Per-subset breakdowns are omitted for test-set rows (marked $\ddagger$) due to low per-subset sample counts. Per-category CIs are in Appendix~\ref{sec:per-cat-ci}; full calibration variant breakdowns are in Appendix~\ref{sec:calibration-detail}.}
...
Soft blend (test)$^\ddagger$ & ${\sim}$343 & 80.2\% ($\pm$4.2) & --- & --- & --- & --- & --- & \$0.082 & 6.1$\times$ \\
Var-inf.\ ($\leq$2, test)$^\ddagger$ & ${\sim}$343 & 74.9\% ($\pm$4.7) & --- & --- & --- & --- & --- & ${\sim}$\$0.022 & 1.6$\times$ \\
\bottomrule
\end{tabular}
...
\caption{Accuracy by condition and category. Criteria $k{=}8$ (83.6\%) matches combined (82.6\%) at lower cost (5.3$\times$ vs 6.8$\times$ baseline); their CIs overlap and we interpret the ordering as a practical tie. Precise~IF remains the hardest category across all conditions.}
...
\caption{Cost vs accuracy Pareto frontier. Criteria $k{=}8$ (83.6\%, 5.3$\times$) is the highest-accuracy point; mini+criteria $k{=}8$ (81.5\%, 1.2$\times$) matches vanilla full-model ensembling (Ensemble $k{=}8$, 81.5\% at 5.0$\times$) at ${\sim}$1/4 the cost (the two are statistically tied, CIs overlap heavily); nano $k{=}8$ sets the low-cost floor. Combined (82.6\%, 6.8$\times$) is dominated by criteria $k{=}8$.}
\label{fig:pareto}
\end{figure*}

\paragraph{Headline results.} The baseline reaches 71.7\% ($\pm$2.1pp) at \$0.013/ex. Criteria injection alone adds +3.0pp at $k{=}1$ (74.7\%; paired bootstrap $p{>}0.999$) with negligible marginal cost --- a few extra input tokens per call, no structural change to the scoring protocol --- and modestly reduces the tie rate at $k{=}1$ (20.4\% baseline $\to$ 17.4\% criteria). Ensembling at $k{=}8$ adds +9.8pp (81.5\% at 5$\times$ cost) and sharply reduces the tie rate (baseline $k{=}1$: 20.4\%, $k{=}8$: 4.5\%), since ties now require all four response means to match exactly across $k$ draws. Together, criteria + ensembling reach \textbf{83.6\%} (+11.9pp over baseline, 5.3$\times$ cost; +11.9pp also on the $N{=}1710$ intersection, Appendix~\ref{sec:intersection}) and further collapse the tie rate to 3.2\%, paired-significant against vanilla ensembling ($p{>}0.999$).
...
\paragraph{Calibration context.} All four calibration variants improve over baseline at $k{=}1$ by +1--2pp. The ``low'' variant (anchoring to a rejected response) slightly outperforms ``high'' (anchoring to the chosen response, 73.8\% vs 72.4\%) --- possibly because the judge finds it easier to distinguish the target from a known-bad anchor than to match a known-good one. Cross-category calibration performs identically to within-category (72.4\%), suggesting the benefit is general scale anchoring rather than category-specific transfer. \textbf{At $k{=}8$, however, calibration provides no additional benefit beyond ensembling alone}: all variants fall within $\pm$0.2pp of vanilla ensembling (81.5\%, cal.\ low: 81.7\%). The $k{=}8$ ensemble already reduces scoring noise enough that the anchoring benefit is redundant --- in contrast to criteria, which provides +2.1pp even at $k{=}8$.
...
Criteria + cal.\ (combined)       & 82.6 ($\pm$1.8) & 79.2 ($\pm$1.9) & --- \\
\bottomrule
\end{tabular}
\end{table}
...
\paragraph{Criteria + ensembling as a simple strong baseline.} Criteria $k{=}8$ (83.6\%) matches the full combined condition (82.6\%) at lower cost (5.3$\times$ vs 6.8$\times$); if you are already paying for $k{=}8$, adding criteria is a near-free +2.1pp upgrade over vanilla ensembling.
