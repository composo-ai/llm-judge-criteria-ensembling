...
GPT-5.4 mini is ${\sim}3\times$ cheaper than the full model but somewhat less accurate. If we could identify which examples the mini model will get wrong, we could route only those to the full model, achieving high accuracy at low cost. We investigate three escalation strategies that use the mini model's per-response score variance as a routing signal: \emph{hard variance routing} (binary per-response routing above a variance threshold), \emph{sigmoid-weighted soft blending} (continuous per-response blend of mini and full scores), and \emph{variance-informed adaptive ensembling} (variable number of full-model calls per response). All strategies are evaluated offline on paired mini/full scores ($k{=}8$ each) collected in a single pass, requiring no additional API calls. Full method descriptions, equations, and variance analysis are in Appendix~\ref{sec:escalation-detail}.
...
\paragraph{Headline results.} The baseline reaches 71.7\% ($\pm$2.1pp) at \$0.013/ex. Criteria injection alone adds +3.0pp at $k{=}1$ (74.7\%) with negligible marginal cost, while $k{=}8$ ensembling adds +9.8pp (81.5\% at 5$\times$ cost). Combined, criteria $k{=}8$ reaches \textbf{83.6\%} (+11.9pp over baseline, 5.3$\times$ cost) --- the best result at its cost level.

\paragraph{Cheaper models benefit disproportionately.} Mini $k{=}8$ achieves 79.2\% at 1.2$\times$ baseline cost; adding criteria pushes it to 81.5\%, \textbf{matching the full model's vanilla $k{=}8$ ensemble at roughly one-quarter the cost}. Nano $k{=}8$ reaches 71.4\% at 0.4$\times$ baseline cost, approaching baseline-level accuracy. Most of the gain is captured by $k{=}3$ for all tiers (Figure~\ref{fig:diminishing-returns}); beyond that, returns diminish.

\paragraph{What doesn't work.} At $k{=}1$, calibration context provides +1--2pp (best variant: ``low'' at 73.8\%). \textbf{At $k{=}8$, calibration provides no additional benefit beyond ensembling alone}: all variants fall within $\pm$0.2pp of the vanilla ensemble (81.5\%, full k=8 cal.\ low: 81.7\%). The combined condition (criteria + calibration + dual-model) reaches 82.6\%, lower than criteria $k{=}8$ alone (83.6\%) at higher cost (6.8$\times$ vs 5.3$\times$). Soft blending achieves 83.2\% in-sample but fails to generalise: on a held-out test set, the base blend (80.2\%) does not beat full $k{=}8$ (81.5\%), indicating midpoint overfitting. Variance-informed routing at low budget (74.9\% at 1.6$\times$ cost) is dominated by mini $k{=}8$ (79.2\% at 1.2$\times$). Full calibration variant results are in Appendix~\ref{sec:calibration-detail}; full escalation analysis is in Appendix~\ref{sec:escalation-detail}.

\paragraph{Robustness.} Sample sizes differ slightly across conditions ($N{=}1700$--$1746$) due to Azure content-filter refusals. Recomputing headline conditions on the intersection of ${\sim}$1710 examples succeeding under all four main collections changes accuracies by at most 0.2pp and preserves the ranking (Appendix~\ref{sec:intersection}). Precise~IF is the hardest category across all conditions (26--53\%); the combined condition provides the largest absolute improvement here (+18.5pp over baseline). A temperature sweep (Appendix~\ref{sec:additional-analyses}) confirms ensembling helps at all temperatures, with gains growing from +4.6pp at temp=0 to +9.8pp at temp=1.0; notably, \texttt{temperature=0} is not deterministic in practice (likely GPU floating-point non-determinism), so ensembling still helps.
...
\paragraph{Practical recommendation.} The highest-value intervention is: (1) write a task-specific criterion and (2) set $k{=}3$--$8$. Neither requires finetuning, and both are drop-in additions to existing pipelines. More complex techniques (calibration, model routing, soft blending) provide diminishing marginal returns over this simple baseline.
...
\begin{itemize}
  ...
  \item We set \texttt{reasoning\_effort="none"} for all conditions, which likely depresses absolute accuracy across the board. Since this is held constant, it does not affect relative comparisons.
  \item We used a single set of author-designed criteria; sensitivity to wording variation is untested.
\end{itemize}

\paragraph{Reproducibility.} All experiments were conducted via Azure OpenAI (GPT-5.4, mini, nano) during March--April 2026. Total API spend was approximately \$1,500. Code and raw data are available at \url{https://github.com/composo-ai/llm-judge-criteria-ensembling}.

\paragraph{Conclusion.} We evaluated five candidate techniques on RewardBench~2. Two techniques dominate the cost--accuracy tradeoff:
\begin{enumerate}
  \item \textbf{Criteria injection}: +3.0pp at $k{=}1$ at essentially zero marginal cost. Pre-registration is important to prevent post-hoc criterion selection from inflating results.
  \item \textbf{Ensembling}: +9.8pp at $k{=}8$; $k{=}3$ captures ${\sim}$70\% of the $k{=}8$ gain at 3/8 the cost. Combined with criteria, ensembling reaches \textbf{83.6\%} (+11.9pp), the best result at its cost level.
\end{enumerate}
Three additional techniques (calibration context, soft blending, variance-informed ensembling) improved over baseline but were dominated by criteria + ensembling at comparable or lower cost. For cost-constrained deployments, \textbf{GPT-5.4 mini with $k{=}8$} achieves 79.2\% at 1.2$\times$ baseline cost; adding criteria pushes it to 81.5\%, matching the full model's vanilla ensemble. The practical recommendation is simple: write a task-specific criterion and set $k{=}3$--$8$. Neither requires finetuning, and both are drop-in additions to any existing LLM judge pipeline.

\paragraph{Future work.} The most important open question is \textbf{cross-model generalisation}: do criteria + ensembling provide similar gains with other judge models (Claude, Gemini, open-weight)?
...
\paragraph{Variance as an error signal.} Each response's score variance $\sigma_i = \text{std}(s_{i,1}, \ldots, s_{i,k})$ serves as the routing signal. Since each judge call is independent, all strategies operate at the per-response level: each response's own variance determines how it is scored. Pearson correlation between per-response variance and correctness is $r = -0.13$: high variance responses are slightly more likely to be judged incorrectly (Figure~\ref{fig:variance-error}). Mini and full model variances are positively correlated ($r = 0.421$), validating mini variance as a proxy for full model uncertainty. Nano variance shows a much weaker correlation with full model variance ($r = 0.106$), suggesting that nano's scoring noise is largely independent (Figure~\ref{fig:variance-corr}).
...
The hard variance routing variant traces a Pareto frontier with a large dead zone in the middle: escalating some but not all responses rarely changes the winner, since accuracy depends on relative rankings across all four responses (Figure~\ref{fig:escalation-pareto}). Meaningful operating points cluster near mini $k{=}8$ (cheap) or full $k{=}8$ (expensive).

\begin{figure}[t]
\centering
\includegraphics[width=\columnwidth]{per_response_escalation_pareto.png}
\caption{Pareto frontier for per-response hard variance routing. Each point is a different threshold $\theta$.}
\label{fig:escalation-pareto}
\end{figure}

On the full dataset (in-sample), soft blending achieves 83.2\% vs 81.5\% for full $k{=}8$ (Figure~\ref{fig:soft-blending-fig}). However, on a held-out test set the base blend (80.2\%) does not beat full $k{=}8$ (81.5\%), indicating midpoint overfitting. The benefit is more robust when combined with criteria + calibration: combined+blend reaches 84.8\% on the test set, outperforming combined full $k{=}8$ (82.6\%).

\begin{figure}[t]
\centering
\includegraphics[width=\columnwidth]{soft_blending.png}
\caption{Per-response soft blending accuracy vs mean blend weight $w$ (full dataset, in-sample). Accuracy peaks at 83.2\% near $w = 0.91$ (mostly full model).}
...
\caption{Pareto frontier for per-response variance-informed ensembling. Gray points: full grid of $(\sigma_1, \sigma_2)$ configurations (train-set). Stars: test-set accuracy for best overall (81.0\%) and budget-constrained (74.9\%) configurations.}
...
\caption{Model agreement with full ($k{=}8$) as a function of ensemble size.}
\label{fig:convergence}
\end{figure}

Mini agreement approaches ${\sim}$79\% by $k{=}5$; the ceiling reflects genuine systematic disagreement between the two models on ${\sim}$20\% of examples. Nano shows a qualitatively similar pattern but with a lower ceiling: agreement rises from ${\sim}$50\% at $k{=}1$ to ${\sim}$70\% at $k{=}8$, with rank correlation plateauing around 0.67 (vs 0.79 for mini).
