...
\begin{abstract}
Using a language model to score or rank candidate responses has become a scalable alternative to human evaluation in reinforcement learning from human feedback (RLHF) pipelines, benchmarking, and application layer evaluations. However, output reliability depends heavily on prompting and aggregation strategy. We present an empirical investigation of four drop-in techniques --- ensemble scoring, task-specific criteria injection, calibration context, and adaptive model escalation --- for improving GPT-5.4 judge accuracy on RewardBench~2, with a unifying lens of noise control on the stochastic judge: ensembling as Monte Carlo averaging over per-call noise, criteria injection as between-response discrimination sharpening, and per-response score variance as an uncertainty signal. Ensemble scoring (+9.8pp) and task-specific criteria injection (+3.0pp, virtually cost free) together reach \textbf{83.6\%} accuracy, +11.9pp over the 71.7\% baseline. Calibration context and adaptive model escalation map additional operating points on the cost--accuracy Pareto frontier. Smaller models benefit disproportionately from ensembling: GPT-5.4 mini at ensemble size $k{=}8$ achieves 79.2\%, making high-accuracy LLM judges accessible at low cost. The recommendation replicates on Claude Sonnet 4.6 (83.5\%) and Claude Haiku 4.5 (86.0\%), supporting cross-family generalisation.
\end{abstract}
...
\caption{Cost vs.\ accuracy Pareto frontier across all evaluated conditions. Criteria $k{=}8$ (83.6\%, 5.3$\times$ baseline cost) is the highest-accuracy point; mini+criteria $k{=}8$ (81.5\%, 1.2$\times$) matches full-model ensembling (81.5\% at 5.0$\times$) at roughly one-quarter the cost; nano $k{=}8$ sets the low-cost floor. Combined (82.6\%, 6.8$\times$) is dominated by criteria $k{=}8$.}
\label{fig:pareto}
\end{figure}
...
\begin{itemize}
  ...
  \item Evidence that criteria injection and ensembling dominate the cost--accuracy tradeoff (Figure~\ref{fig:pareto}), reaching 83.6\% (+11.9pp, identical on the $N{=}1710$ intersection) at 5.3$\times$ cost, while calibration, model routing, and soft blending improved over baseline but were dominated by criteria + ensembling at comparable or lower cost.
...
\paragraph{Baseline.} The baseline condition applies the RB2 prompt verbatim with $k{=}1$ completion per response (four API calls per example, all using the full GPT-5.4 model). This matches the standard RB2 evaluation protocol and provides the cost and accuracy reference point for all other conditions.
...
\paragraph{Motivation.} GPT-5.4 mini is ${\sim}3\times$ cheaper than the full model but somewhat less accurate. If we could identify which examples the mini model will get wrong, we could route only those to the full model. Per-response score variance is a plausible routing signal: variance correlates weakly but systematically with correctness ($r{=}{-}0.13$; AUC${=}0.60$ as an incorrectness classifier), and mini-model variance tracks full-model variance ($r{=}0.421$; nano's correlation is weaker at $r{=}0.106$, Appendix~\ref{sec:escalation-detail}).
...
\toprule
Condition & $N$ & Overall (95\% CI) & Fact. & Focus & Math & P-IF & Safety & vs Base \\
\midrule
\multicolumn{9}{l}{\textit{(a) Recommended techniques}} \\
\midrule
Baseline (full $k{=}1$)   & 1729 & 71.7\% ($\pm$2.1) & 76.4 & 70.1 & 61.2 & 34.0 & 87.3 & 1.0$\times$ \\
Criteria (full $k{=}1$)   & 1738 & 74.7\% ($\pm$2.1) & 77.9 & 72.3 & 73.2 & 32.1 & 90.6 & 1.1$\times$ \\
Ensemble (full $k{=}8$)   & 1730 & 81.5\% ($\pm$1.8) & 86.7 & 81.8 & 74.9 & 44.7 & 92.1 & 5.0$\times$ \\
\textbf{Criteria (full $k{=}8$)} & 1741 & \textbf{83.6\%} ($\pm$1.7) & \textbf{89.1} & \textbf{82.8} & \textbf{79.2} & 48.8 & \textbf{93.2} & 5.3$\times$ \\
Mini model $k{=}8$        & 1730 & 79.2\% ($\pm$2.0) & 83.3 & 80.2 & 68.3 & 40.3 & 92.8 & 1.2$\times$ \\
Criteria (mini $k{=}8$)   & 1742 & 81.5\% ($\pm$1.9) & 85.5 & 82.2 & 72.7 & 41.2 & 94.9 & 1.2$\times$ \\
Nano model $k{=}8$        & 1705 & 71.4\% ($\pm$2.1) & 67.9 & 74.5 & 61.2 & 42.4 & 87.6 & 0.4$\times$ \\
\midrule
\multicolumn{9}{l}{\textit{(b) Investigated techniques}} \\
\midrule
Calibration low ($k{=}8$)   & 1744 & 81.7\% ($\pm$1.8) & 86.7 & 80.6 & 75.4 & 46.9 & 93.0 & 5.6$\times$ \\
Combined (full $k{=}8$)     & 1746 & 82.6\% ($\pm$1.8) & 87.6 & 80.6 & 77.6 & \textbf{52.5} & 92.8 & 6.8$\times$ \\
Soft blend (test)$^\ddagger$ & ${\sim}$343 & 80.2\% ($\pm$4.2) & --- & --- & --- & --- & --- & 6.1$\times$ \\
Variance-informed (budget $\leq$2, test)$^\ddagger$ & ${\sim}$343 & 74.9\% ($\pm$4.7) & --- & --- & --- & --- & --- & 1.6$\times$ \\
\bottomrule
\end{tabular}
...
To test whether the criteria + ensembling recommendation transfers beyond GPT, we collected identical \texttt{base} and \texttt{criteria} runs on \textbf{Claude Sonnet 4.6} and \textbf{Claude Haiku 4.5} via the Claude CLI (1{,}763 RB2 examples per condition at $k{=}8$). The dominated techniques (calibration, escalation, soft blending) were not collected for Claude --- the cross-model claim is intentionally scoped to the recommended pair.

\begin{figure}[t]
\centering
\includegraphics[width=\columnwidth]{cross_model_gain.png}
\caption{Cross-model accuracy on RewardBench~2: baseline ($k{=}1$), ensemble only ($k{=}8$), and criteria + ensembling ($k{=}8$). The recommended configuration improves on baseline across all four model conditions; deltas (criteria + ensemble vs baseline) are annotated above the right-most bar in each group.}
\label{fig:cross-model}
\end{figure}

\begin{table}[ht]
\centering
\caption{Cross-model headline accuracy (\%, 95\% bootstrap CI half-widths). $\Delta$ is criteria + ensembling vs baseline.}
\label{tab:cross-model}
\small
\begin{tabular}{lccc}
\toprule
Model              & Base.\ ($k{=}1$) & Crit.+ens.\ ($k{=}8$) & $\Delta$ \\
\midrule
GPT-5.4            & 71.7 ($\pm$2.1) & 83.6 ($\pm$1.8)          & $+$11.9 \\
GPT-5.4 mini       & 64.8 ($\pm$2.3) & 81.5 ($\pm$1.9)          & $+$16.7 \\
Claude Sonnet 4.6  & 72.3 ($\pm$2.2) & 83.4 ($\pm$1.8)          & $+$11.1 \\
Claude Haiku 4.5   & 72.3 ($\pm$2.1) & \textbf{85.8 ($\pm$1.7)} & $+$13.5 \\
\bottomrule
\end{tabular}
\end{table}

\paragraph{Recommendation generalises.} Criteria + ensembling produces a paired-significant gain over the $k{=}1$ baseline for both Claude models (paired bootstrap, $p{>}0.999$ in each case). Sonnet 4.6 reaches 83.4\%, statistically tied with GPT-5.4's 83.6\%; Haiku 4.5 reaches \textbf{85.8\%}, the highest accuracy in our cross-model panel. The within-family pattern of "smaller models gain more from ensembling" replicates inside the Claude family: Haiku gains $+$13.5pp where Sonnet gains $+$11.1pp. The same tie-rate collapse from $k{=}1$ to $k{=}8$ that drives GPT's gain (Section~\ref{sec:discussion}) replicates on Claude (Appendix~\ref{sec:cross-model-detail}), supporting the noise-control framing as model-family-agnostic.

\paragraph{Caveats.} Claude data was collected via the Claude CLI (subprocess), not the Anthropic API directly, with no \texttt{reasoning\_effort} parameter (Anthropic does not expose one); determinism characteristics may differ from the Azure pipeline. We test only Sonnet 4.6 and Haiku 4.5; Opus and other model families (Gemini, open-weight) remain untested. The dominated-techniques generalisation (calibration, escalation, soft blending) is not assessed across providers.
...
\begin{itemize}
  \item We evaluate three judge model families (GPT-5.4, Claude Sonnet 4.6, Claude Haiku 4.5); coverage of Gemini, open-weight judges, and finetuned judges (e.g., Prometheus 2) remains untested.
...
\paragraph{Conclusion.} On RewardBench~2, all four drop-in techniques we investigate improve over baseline. \textbf{Criteria injection} (+3.0pp at $k{=}1$, near-free) and \textbf{ensembling} (+9.8pp at $k{=}8$, with $k{=}3$ capturing ${\sim}$70\% of the gain) are the simplest and most cost-effective; together they reach \textbf{83.6\%} (+11.9pp), and we recommend them in practice for LLM-as-a-judge evaluation. The recommendation replicates on Claude Sonnet 4.6 (83.4\%) and Haiku 4.5 (85.8\%), supporting cross-family generalisation (Section~\ref{sec:cross-model}). Calibration context and adaptive model escalation also improve over baseline but at higher cost, and at high $k$ variance-reduction techniques become substitutes rather than complements, so stacking did not yield additive gains.
...
\subsection{Cross-Model Generalisation Details}
\label{sec:cross-model-detail}

\paragraph{Diminishing returns across model families.} Figure~\ref{fig:cross-model-dim} extends the diminishing-returns curve to Claude. The shape is qualitatively similar across providers --- most of the gain is captured by $k{=}3$ for all five models, consistent with Monte Carlo variance reduction not being provider-specific.

\begin{figure}[ht]
\centering
\includegraphics[width=\columnwidth]{cross_model_diminishing.png}
\caption{Accuracy vs.\ ensemble size $k$ for the five models in our cross-model panel. The diminishing-returns shape replicates across providers; absolute levels differ but the curvature does not.}
\label{fig:cross-model-dim}
\end{figure}

\paragraph{Tie-rate collapse on Claude.} Both Claude models reproduce the GPT signature of $k{=}1 \to k{=}8$ tie-rate collapse (Table~\ref{tab:cross-model-tie}): ties become rare under ensembling because they require all four response means to coincide across $k$ draws. The relative reduction ($\sim$5--7$\times$) matches what we observe on GPT-5.4 (20.4\% $\to$ 4.5\%).

\begin{table}[ht]
\centering
\caption{Tie rates ($k{=}1$ vs $k{=}8$) for the Claude collections.}
\label{tab:cross-model-tie}
\small
\begin{tabular}{lcc}
\toprule
Condition           & $k{=}1$ & $k{=}8$ \\
\midrule
Sonnet baseline     & 18.5\% & 4.0\% \\
Sonnet criteria     & 18.7\% & 4.7\% \\
Haiku baseline      & 18.3\% & 2.6\% \\
Haiku criteria      & 17.0\% & 2.5\% \\
\bottomrule
\end{tabular}
\end{table}

\paragraph{Paired bootstrap.} Criteria + ensembling at $k{=}8$ vs.\ baseline $k{=}1$ on the intersection of examples: paired-significant for all four model conditions (paired bootstrap $n{=}2000$). Sonnet $\Delta{=}{+}11.0$pp [95\% CI $+9.1, +12.8$], $p{>}0.999$; Haiku $\Delta{=}{+}13.3$pp [$+11.5, +15.2$], $p{>}0.999$.

\paragraph{Collection methodology.} Claude data was collected via the Claude CLI in subprocess mode (driver \texttt{collect\_claude.py}) with default sampling settings (temperature 1.0; no \texttt{reasoning\_effort} parameter, which Anthropic does not expose). The CLI handles auto-update behaviour internally. Rate-limit errors retry indefinitely with exponential backoff; transient errors retry up to 5 times. Per-example refusals are tracked separately and excluded from accuracy computation, mirroring the Azure OpenAI content-filter handling on the GPT side.
