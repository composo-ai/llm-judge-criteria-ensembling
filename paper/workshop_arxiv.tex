...
% ICML 2026 workshop submission --- arXiv (non-anonymous) version.
% For double-blind submission: \usepackage{icml2026}
% For accepted camera-ready: \usepackage[accepted]{icml2026}
% For arXiv preprint (shows authors, no PMLR proceedings notice): \usepackage[preprint]{icml2026}
...
\caption{Accuracy by condition and category. Each bar shows the best-performing provider for that class+condition; per-row provider attribution is in Table~\ref{tab:results-main}. Error bars are 95\% bootstrap confidence intervals (2{,}000 resamples) on the per-category accuracy; smaller categories (Math, Precise~IF) have correspondingly wider intervals, and full per-category CIs are tabulated in Appendix~\ref{sec:per-cat-ci}. Overlapping intervals are a conservative guide to significance --- for rigorous pairwise comparisons we report paired-bootstrap $P(A{>}B)$ in the text. $k{=}8$ + criteria (mini) reaches the highest overall accuracy (85.8\%); $k{=}8$ + criteria (full) at 83.6\% matches $k{=}8$ + combined (full) at 82.6\% at lower cost. Precise IF remains the hardest category across all conditions.}
