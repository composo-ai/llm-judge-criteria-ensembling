...
\caption{Accuracy by condition and category. Each bar shows the best-performing provider for that class+condition; per-row provider attribution is in Table~\ref{tab:results-main}. $k{=}8$ + criteria (mini) reaches the highest overall accuracy (85.8\%); $k{=}8$ + criteria (full) at 83.6\% matches $k{=}8$ + combined (full) at 82.6\% at lower cost. Precise IF remains the hardest category across all conditions.}
